...
모든 실습에 하나의 깨끗한 가상 환경(Virtual Environment)을 사용하십시오. 본 설정은 Linux 배포판 및 Windows의 Python 3.12 환경에서 테스트되었습니다. CAB H 56 / CAB H 57에 있는 Linux 컴퓨터를 사용할 수도 있습니다.
...
코드를 깔끔하고 가독성 있게 유지하십시오. 제출하기 전에 모든 코드가 정상적으로 실행되는지 확인하십시오.

### 비디오 지침 (Video Guidelines)

비디오의 목적은 구현한 알고리즘에 대한 이해도를 보여주는 것입니다.

- 설명은 **명확하고 간결하게** 유지하십시오.
...
2. **이론 질문:** 비디오에 이론 질문에 대한 답변이 반드시 포함되어야 합니다.
...
선택 사항으로, 다음을 실행하여 에이전트의 비디오를 녹화할 수 있습니다:

```bash
python scripts/eval_dqn.py --record_video
```

주의: `--play`와 `--record_video` 옵션은 서로 다른 렌더링 모드를 요구하므로 동시에 사용할 수 없습니다.
...
2. **결과:** 제공된 스크립트를 실행하여 생성된 학습 곡선(Training Curve) 및 출력된 평가 요약본. 모든 그림을 하나의 PDF 파일로 합쳐서 제출하십시오.
3. **이론 질문:** 비디오에 이론 질문에 대한 답변이 반드시 포함되어야 합니다.
...
`exercises/ex3_ppo_student.py` 파일의 TODO 부분을 채우십시오:
...
### 제출물 (Deliverables)
1. **코드:** `exercises/ex3_ppo_student.py`에서 TODO를 완료한 구현 코드.
2. **결과:** 텐서보드의 모든 학습 곡선 및 제공된 스크립트를 실행하여 생성된 출력된 평가 요약본. 모든 그림을 하나의 PDF 파일로 합쳐서 제출하십시오.
3. **이론 질문:** 비디오에 이론 질문에 대한 답변이 반드시 포함되어야 합니다.
...
`exercises/ex4_sac_student.py` 파일의 TODO 부분을 채우십시오:
...
### 제출물 (Deliverables)
1. **코드:** `exercises/ex4_sac_student.py`에서 TODO를 완료한 구현 코드.
2. **결과:** 텐서보드의 모든 학습 곡선 및 제공된 스크립트를 실행하여 생성된 출력된 평가 요약본. 모든 그림을 하나의 PDF 파일로 합쳐서 제출하십시오.
3. **이론 질문:** 비디오에 이론 질문에 대한 답변이 반드시 포함되어야 합니다.
...
Gradescope의 **Homework 4** 과제에 다음 파일들을 제출하십시오:
...
파일 이름을 변경하거나 함수 시그니처를 수정하지 **마십시오**. 업로드 시 오토그레이더(Autograder)가 구현을 검증합니다.

### 비디오 제출

모든 실습을 포함하는 하나의 비디오 파일(`.mp4`)을 제출하십시오. 파일 크기가 너무 커서 제출할 수 없는 경우 여러 파일로 분할할 수 있으나, 순서를 명확히 표시해 주십시오.

- **최대 분량: 5분.** 제한 시간을 초과하면 감점됩니다. 배속 재생(예: 1.5배속, 2배속 재생)은 허용되지 않습니다.
- 대략적인 가이드라인으로, 실습 1~3은 각각 약 1분, 실습 4는 2분을 목표로 하십시오.
- 설명을 돕기 위해 슬라이드, 그래프 또는 화면 녹화 자료를 사용할 수 있습니다.
- 설명하는 동안 결과물이 **화면에 보여야** 합니다.
- 제출하기 전에 비디오와 오디오를 확인하십시오.

### PDF 제출
실습 2, 3, 4에 대한 PDF 파일을 제출하십시오. 실습 1의 경우, 이론 질문에 답하기 위해 비디오 제출물에서 모든 이미지를 시각화해야 하므로 PDF 파일은 필요하지 않습니다.