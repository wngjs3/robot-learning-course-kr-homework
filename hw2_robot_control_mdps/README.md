...
## Deliverables
1. **비디오:** 로봇이 생성된 키포인트를 추적하는 비디오(.mp4). 로봇의 움직임과 이론 질문을 포함하여 **비디오 길이는 2분 미만이어야 합니다.**
2. **코드:** `exercises/ex1.py`에서 TODO를 채운 코드.
...
비디오에서 이러한 동작 변화를 직접 보여줄 필요는 없으며, 비디오에 답변을 텍스트로 적거나 말로 설명하면 됩니다.
이론 질문은 짧고 직접적인 답변만 요구합니다. 각 질문은 1문장으로 답변하는 것을 권장합니다.

## Deliverables
1. **비디오:** 로봇이 경유지 사이를 이동하는 비디오(.mp4). 로봇의 움직임과 이론 질문을 포함하여 **비디오 길이는 2분 미만이어야 합니다.**
2. **코드:** `exercises/ex2.py`에서 TODO를 채운 코드.
3. **이론 질문:** 비디오에 이론 질문에 대한 답변이 포함되어야 합니다.
...
시뮬레이션 창이 나타납니다. 로봇은 10개의 에피소드(각 에피소드는 2초로 설정됨) 동안 추적을 수행합니다. 최종 ee_tracking_error가 터미널에 출력되며, 10개 에피소드가 끝날 때 평균값이 포함됩니다. **이 출력 화면을 비디오에 포함하십시오.**
훈련된 정책은 **평균 최종 EE 추적 오차 < 0.05**인 경우 만점을 받습니다. 정책이 제대로 수렴한다면 이는 쉽게 달성할 수 있습니다.
...
로봇이 레미니스케이트 곡선 위의 키포인트를 추적할 때 어떤 차이점을 관찰할 수 있습니까? RL 정책의 성능을 향상시키기 위해 ex3의 함수들을 어떻게 변경할 수 있습니까? 이 함수들을 수정하십시오(함수의 인자를 변경하고 `env/so100_tracking_env.py`에서도 그에 맞게 변경할 수 있습니다). 새로운 환경으로 다른 RL 정책을 훈련하고 비디오에서 성능을 보여주며, 여러분의 변경 사항이 로봇의 성능에 어떤 영향을 미쳤는지 설명하십시오. PPO 하이퍼파라미터(gamma, ent_coef 등)를 변경할 수도 있습니다.

## Deliverables
1. **비디오:** 터미널의 오차 출력 화면을 포함하여 무작위 목표 사이를 이동하는 로봇의 비디오(.mp4) **(30초 미만)**. 보너스 질문을 완료한 경우, 이론 질문에 대한 답변과 새 정책의 성능을 포함하십시오 **(1분 미만)**.
2. **코드:** `exercises/ex3.py`에서 TODO를 채운 코드.
3. **보너스 질문:** 보너스 점수를 받으려면 비디오에 이론 질문에 대한 답변이 포함되어야 하며, 코드에 여러분의 수정 사항이 포함되어야 합니다.
